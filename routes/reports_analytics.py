...
async def get_reports_dashboard(
    ...
    try:
        ...
        # ── Leads data (from customers created that month) ───────────────
        leads_query = text("""
            SELECT
        ...
        # ── Monthly trend (last 6 months from the selected month) ────────
        ...
        if use_v2:
            trend_query = text("""
                ...
                WHERE agent_id = :agent_id
                  ...
                  AND created_at < (:end_date::date + INTERVAL '1 month')
            ...
            trend_query = text("""
                ...
                FROM policies
                WHERE agent_id = :agent_id
                  AND COALESCE(issue_date, start_date) >= :start_date::date
                  AND COALESCE(issue_date, start_date) < (:end_date::date + INTERVAL '1 month')
...
async def get_reports_policies(
    ...
    """
    Returns filtered list of policies for a given month with customer name, amount, commission.
    ...
    try:
        ...
        if use_v2:
            ...
            query = text("""
                ...
                WHERE agent_id = :agent_id
                  AND EXTRACT(YEAR FROM COALESCE(issue_date, start_date)) = :year
                GROUP BY EXTRACT(MONTH FROM COALESCE(issue_date, start_date)), TO_CHAR(COALESCE(issue_date, start_date), 'YYYY-MM')
                ORDER BY month_num ASC
            """)
